...
**Employee invitations** built 2026-06-12 (`feat/employee-invitations`, stacked on course-assignments): migration `20260612200000` adds `organization_members.invite_token` (unique) + `private.enforce_seat_limit()` BEFORE INSERT trigger (counts all members per org; 0 = unlimited — closes the Phase 0 gap where seat_limit was informational only). Company_admin invites by email + role (learner/company_admin) on `/admin/companies/[id]`; invite link `/invite/<token>`; accept flow has two paths — signed-in user with matching email links their account, or new visitor sets a password → `admin.createUser({email_confirm:true})` + auto sign-in (service-role client in `lib/supabase/admin.ts`, used only here). Resend email via `lib/email/invitation.ts` (dev mode: `onboarding@resend.dev`, deliverable only to the Resend account owner — email failure is non-fatal, admin gets the link to share manually; verified domain needed for real sends → set `RESEND_FROM_EMAIL`). **Browser-verified 2026-06-12**: invite create → «Invited» row + link + dev-mode notice; accept (new account + password) → dashboard, DB row active+linked; invite link single-use (shows «Invitation not found» after accept); revoke invitation with confirm → row gone; seat-limit trigger → P0001 when full (tested via SQL). Security advisors: no new findings (`enforce_seat_limit` is in `private` schema, fixed search_path).
Dev environment fully configured: `.env.local` has all Supabase + Stripe keys set, **`RESEND_API_KEY` now set** (added 2026-06-12). Dev server runs on port 3001. Supabase Site URL set to :3001. Stripe account business name set ("LMS Test", test mode). `BUNNY_*` still **empty** — last v1 blocker.
Notion status page: https://www.notion.so/366fbb2a781f81ff929ae0472e66fb08

### Next tasks
- Merge PR #9 (course assignments — verified) then PR for employee invitations (stacked, verified)
- Company report: per-employee completion % + quiz scores, CSV export (main sellable artifact — now unblocked by assignments)
- Resend: verify a sending domain + set `RESEND_FROM_EMAIL` for real invite delivery (dev mode only reaches the account owner)
- Bunny signed video URLs (blocked: Bunny credentials — last v1 blocker)
- Handle implicit-flow hash tokens on /login (magic link + password reset; spawned task)
